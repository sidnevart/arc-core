...
import { applyChatScale as applySharedChatScale, normalizeChatScalePercent, CHAT_SCALE_LIMITS } from "/shared/density.js";
import { renderMarkdown } from "/shared/markdown.js";

const RECENT_PROJECTS_KEY = "arc.desktop.preview.recentProjects";
const CLIENT_LOGS_KEY = "arc.desktop.preview.clientLogs";
const ATTACHED_SESSIONS_KEY = "arc.desktop.preview.attachedSessions";
const CHAT_SCALE_KEY = "arc.desktop.preview.chatScalePercent";
const MAX_CLIENT_LOGS = 100;

const state = {
  locale: normalizeLocale(window.localStorage.getItem("arc.desktop.preview.locale") || "ru"),
  chatScalePercent: normalizeChatScalePercent(window.localStorage.getItem(CHAT_SCALE_KEY) || 100),
  currentScreen: "chat",
  showWelcome: true,
  path: window.localStorage.getItem("arc.desktop.preview.path") || "",
  draftPath: window.localStorage.getItem("arc.desktop.preview.path") || "",
  recentProjects: parseStoredList(window.localStorage.getItem(RECENT_PROJECTS_KEY)),
  projectState: null,
  workspace: null,
  providers: [],
  agents: [],
...
const M = {
  ru: {
    ...
    "chat.detailsHint": "Здесь видны файлы проекта и файлы ARC, которые ты добавил в разговор.",
    ...
    "chat.placeholder": "Например: объясни клеточное дыхание через схему и мини-симуляцию.",
    ...
    "chat.noProjectMaterials": "Пока в ARC нет загруженных файлов.",
    "chat.upload": "Загрузить материалы",
    "chat.uploadInline": "Добавить файлы",
    "chat.uploaded": "Файлы загружены в ARC.",
    "chat.uploadBusy": "Загружаю файлы в ARC",
    "chat.materialAttachHint": "Загруженные файлы сразу становятся частью контекста ARC. Если разговор уже открыт, ARC привяжет их и к текущей теме.",
    ...
    "chat.inspectorEmpty": "Открой детали чата, чтобы посмотреть материалы этой темы и файлы проекта.",
    ...
    "chat.openDocument": "Открыть документ в деталях чата",
    ...
    "files.arc": "Файлы ARC",
    "files.empty": "Пока не найдено файлов для просмотра.",
    "files.delete": "Удалить из ARC",
    ...
    "live.windowLead": "ARC держит живые демо и миниаппы внутри приложения, без обязательного перехода в браузер.",
  ...
  en: {
    ...
    "chat.readyLabel": "Ready",
    "chat.demosLabel": "Demo",
    "chat.detailsLabel": "Files",
    "chat.outputs": "Ready now",
    ...
    "chat.placeholder": "For example: explain cellular respiration with a diagram and mini simulation.",
    ...
    "chat.inspectorEmpty": "Open chat details to inspect this topic's materials and project files.",
    ...
    "chat.openDocument": "Open document on the right",
    ...
    "learn.bio.summary": "Ask the agent to explain how a cell turns glucose into energy with a diagram and mini simulation.",
    "learn.project.title": "Explain this project",
    "learn.project.summary": "Ask for a plain-language explanation of what the project does and where to start.",
    ...
    "files.arc": "ARC files",
    "files.empty": "No files found yet.",
    "files.delete": "Delete from ARC",
    "attached.title": "Attached context",
    "attached.empty": "You can pull a past session here and continue with its context.",
...
const t = (key) => M[state.locale]?.[key] || M.en[key] || key;
...
function persistClientLogs() { window.localStorage.setItem(CLIENT_LOGS_KEY, JSON.stringify(state.clientLogs.slice(-MAX_CLIENT_LOGS))); }
function persistChatScale() { window.localStorage.setItem(CHAT_SCALE_KEY, String(state.chatScalePercent)); }
function applyChatScaleState() {
  state.chatScalePercent = applySharedChatScale(window, state.chatScalePercent);
  persistChatScale();
}
function pushClientLog(level, message) {
  state.clientLogs = [...state.clientLogs.slice(-(MAX_CLIENT_LOGS - 1)), { level, message: String(message || ""), created_at: new Date().toISOString() }];
  persistClientLogs();
}
function showBanner(message, kind = "") {
  byId("status-banner").innerHTML = message ? `<div class="banner ${kind}">${escapeHTML(message)}</div>` : "";
}
...
function queryPath() { return `path=${encodeURIComponent(state.path)}`; }

const transport = {
  projectState: (path) => getJSON(`/api/project-state?path=${encodeURIComponent(path || "")}`),
  initWorkspace: (req) => postJSON("/api/init-workspace", req),
  setWorkspaceMode: (req) => postJSON("/api/workspace-mode", req),
  developerAccess: (path) => getJSON(`/api/developer-access?path=${encodeURIComponent(path || "")}`),
  setDeveloperRole: (req) => postJSON("/api/developer-role", req),
  providers: () => getJSON("/api/providers"),
  agents: () => getJSON("/api/agents"),
  allowedActions: (path, mode, sessionID) => getJSON(`/api/allowed-actions?path=${encodeURIComponent(path || "")}&mode=${encodeURIComponent(mode || "")}&session_id=${encodeURIComponent(sessionID || "")}`),
  sessions: (path, query, mode, status) => getJSON(`/api/sessions?path=${encodeURIComponent(path)}&query=${encodeURIComponent(query || "")}&mode=${encodeURIComponent(mode || "")}&status=${encodeURIComponent(status || "")}`),
  session: (path, sessionID) => getJSON(`/api/session?path=${encodeURIComponent(path)}&session_id=${encodeURIComponent(sessionID)}`),
  testingScenarios: (path) => getJSON(`/api/testing-scenarios?path=${encodeURIComponent(path || "")}`),
  startTesting: (req) => postJSON("/api/testing-start", req),
  testingStatus: (path, runID) => getJSON(`/api/testing-status?path=${encodeURIComponent(path || "")}&run_id=${encodeURIComponent(runID)}`),
  testingControl: (req) => postJSON("/api/testing-control", req),
  verificationProfiles: (path) => getJSON(`/api/verification-profiles?path=${encodeURIComponent(path || "")}`),
  startVerification: (req) => postJSON("/api/verification-start", req),
  verificationStatus: (path, runID) => getJSON(`/api/verification-status?path=${encodeURIComponent(path || "")}&run_id=${encodeURIComponent(runID)}`),
  chatStart: (req) => postJSON("/api/chat-start", req),
  chatSend: (req) => postJSON("/api/chat-send", req),
  taskPlan: (req) => postJSON("/api/task-plan", req),
  taskRun: (req) => postJSON("/api/task-run", req),
  workspaceFile: (path, file) => getJSON(`/api/workspace-file?path=${encodeURIComponent(path)}&file=${encodeURIComponent(file)}`),
  workspaceFiles: (path, limit = 400) => getJSON(`/api/workspace-files?path=${encodeURIComponent(path)}&limit=${encodeURIComponent(limit)}`),
  projectMaterials: (path) => getJSON(`/api/project-materials?path=${encodeURIComponent(path)}`),
  uploadProjectMaterials: (req) => postJSON("/api/project-materials-upload", req),
  deleteProjectMaterial: (req) => postJSON("/api/project-materials-delete", req),
  liveApps: (path, sessionID) => getJSON(`/api/live-apps?path=${encodeURIComponent(path)}&session_id=${encodeURIComponent(sessionID || "")}`),
  startMaterialLiveApp: (req) => postJSON("/api/live-app-start", req),
  startLessonDemo: (req) => postJSON("/api/live-app-start", req),
  ensureLiveApp: (req) => postJSON("/api/live-app-ensure", req),
  stopLiveApp: (req) => postJSON("/api/live-app-stop", req),
...
function isReady() { return state.projectState?.state === "ready"; }
function currentProvider() { return state.workspace?.default_provider || state.projectState?.workspace?.default_provider || "codex"; }
function projectDefaultMode() { return state.workspace?.mode || state.projectState?.workspace?.mode || "work"; }
...
function persistAttachedSessions() { window.localStorage.setItem(ATTACHED_SESSIONS_KEY, JSON.stringify(state.attachedSessionIDs)); }
...
function materialTypeLabel(type) { return t(`result.${type}`) || type || ""; }
function liveStatusLabel(status) {
  switch (status) {
    case "ready": return t("live.ready");
    case "starting": return t("live.starting");
    case "failed": return t("live.failed");
    case "stopped": return t("live.stopped");
...
function renderLiveAppsList(apps) {
  if (!apps?.length) return emptyBlock(t("live.empty"));
...
function renderProjectMaterialsList() {
  ...
  if (!items.length) return emptyBlock(t("chat.noProjectMaterials"));
...
function renderLiveWorkStatus(detail) {
  ...
  if (detail.session.status !== "running" && !transcript && !stderr) return "";
  return `
...
function currentLiveAction(appID = "", origin = "") {
  ...
  if (originKey && state.liveActionState[originKey]) return state.liveActionState[originKey];
  const appKey = liveActionKey(appID, "");
  if (appKey && state.liveActionState[appKey]) return state.liveActionState[appKey];
  return null;
}
...
function livePhase(detail, transcript, stderr) {
  ...
  if (runtime.some((item) => item.status === "starting")) return state.locale === "en" ? "Launching miniapp" : "Поднимает миниапп";
  if (lower.includes("mermaid") || lower.includes("<svg") || lower.includes("diagram")) return state.locale === "en" ? "Preparing diagram" : "Готовит схему";
  if (lower.includes("<html") || lower.includes("simulation") || lower.includes("demo")) return state.locale === "en" ? "Building miniapp" : "Собирает миниапп";
...
function renderInlineVisualOutputs(detail) {
  const materials = (detail?.materials || []).filter((item) => isVisualMaterial(item));
  const liveApps = (detail?.live_apps || detail?.liveApps || []).filter((item) => item.status === "ready" || item.status === "starting" || item.status === "stopped");
  if (!materials.length && !liveApps.length) return "";
  return `
    <div class="inline-visual-stack">
      <div class="thread-output-label">${escapeHTML(t("chat.inlineVisuals"))}</div>
      <div class="inline-visual-grid">
        ${materials.slice(0, 3).map((item) => renderInlineVisualMaterial(item, detail)).join("")}
        ${liveApps.slice(0, 2).map((item) => renderInlineLiveApp(item)).join("")}
      </div>
    </div>
  `;
}
...
function renderStatic() {
  ...
  document.querySelectorAll(".nav-item, .topbar-nav-item").forEach((button) => {
    if ((button.id === "nav-testing" || button.id === "topbar-testing") && !testingVisible) return;
    button.classList.toggle("active", button.dataset.screen === state.currentScreen);
    const needsProject = button.dataset.requiresProject === "true";
    button.disabled = needsProject && !isReady();
    button.classList.toggle("disabled", needsProject && !isReady());
  });
  applyChatScaleState();
  if (byId("locale-select")) {
    byId("locale-select").value = state.locale;
  }
  if (byId("project-chip")) {
    byId("project-chip").textContent = state.projectState?.name || "ARC не открыт";
  }
  if (byId("project-subtitle")) {
    byId("project-subtitle").textContent = isReady() ? (state.projectState?.message || t("project.ready")) : (state.projectState?.message || t("project.missing"));
  }
  if (byId("project-default-agent-inline")) {
    byId("project-default-agent-inline").innerHTML = isReady()
      ? renderAgentMenu("project", projectDefaultMode(), { topbar: true })
      : "";
  }
}
...
function renderWelcome() {
  ...
  const visible = state.showWelcome || !isReady();
  overlay.className = visible ? "welcome-overlay visible" : "welcome-overlay";
  if (!visible) { overlay.innerHTML = ""; return; }
  overlay.innerHTML = `
    <div class="welcome-dialog">
      <div class="eyebrow">ARC Preview</div>
      ...
      <label class="metric-label">${escapeHTML(t("welcome.path"))}</label>
      <div class="path-row">
      ...
      ${state.projectState ? `<div class="welcome-state ${escapeHTML(state.projectState.state)}"><strong>${escapeHTML(state.projectState.state === "ready" ? t("welcome.ready") : t("welcome.notReady"))}</strong><div class="muted">${escapeHTML(state.projectState.message || "")}</div></div>` : ""}
      <div class="button-row">${state.projectState?.state === "selected_not_initialized" ? `<button id="welcome-init" class="primary-button">${escapeHTML(t("welcome.setup"))}</button>` : ""}</div>
      <div class="section-gap"><strong>${escapeHTML(t("welcome.recent"))}</strong></div>
      ${state.recentProjects.length ? `<div class="recent-projects">${state.recentProjects.map((path) => `<button class="recent-project-button" data-recent-project="${escapeHTML(path)}">${escapeHTML(summarizeProjectPath(path))}</button>`).join("")}</div>` : `<div class="empty">${escapeHTML(t("welcome.noRecent"))}</div>`}
  ...
  `;
  byId("welcome-path-input").addEventListener("input", (event) => { state.draftPath = event.target.value; });
  byId("welcome-open").onclick = () => openProject(state.draftPath);
  byId("welcome-init")?.addEventListener("click", () => initWorkspace());
  overlay.querySelectorAll("[data-recent-project]").forEach((button) => button.addEventListener("click", () => openProject(button.dataset.recentProject)));
}

function renderSessionTabButtons() {
  const inspectorTab = currentInspectorTab();
  return `<div class="tab-row">${sessionTabs().map((tab) => `<button class="tab-button ${inspectorTab === tab.id ? "active" : ""}" data-session-tab="${escapeHTML(tab.id)}">${escapeHTML(tab.label)}</button>`).join("")}</div>`;
}

function renderMaterialPreview(material) {
  if (!material) return emptyBlock(t("chat.inspectorEmpty"));
  const metaBits = [
  ...
  ` : "";
  const filePreview = state.selectedFileDetail ? `<div class="section-gap"><strong>${escapeHTML(t("file.preview"))}</strong><pre class="code-block">${escapeHTML(state.selectedFileDetail.content || "")}</pre></div>` : "";
  return `
...
function renderChatLiveRefresh() {
  if (state.currentScreen !== "chat" || !isReady()) return;
...
function renderTesting() {
  ...
  const testingCard = !run ? card(t("testing.current"), emptyBlock(t("testing.empty")), "card--subtle") : card(run.title, `
      ...
      <div class="list">${(run.steps || []).map((step, index) => `<div class="list-button static ${index === run.current_step ? "active" : ""}"><div class="list-title"><span>${escapeHTML(step.title)}</span><span>${escapeHTML(step.status)}</span></div><div class="body-copy">${escapeHTML(step.summary)}</div>${step.details ? `<div class="muted">${escapeHTML(step.details)}</div>` : ""}</div>`).join("")}</div>
      <div class="button-row">
      ...
      ${currentStep?.preview_url || currentStep?.previewURL ? `<iframe class="preview-frame" src="${escapeHTML(currentStep.preview_url || currentStep.previewURL)}"></iframe>` : ""}
    `, "card--command");
  const verificationCard = !verificationRun ? card(t("verification.current"), emptyBlock(t("verification.empty")), "card--subtle") : card(verificationRun.title, `
      ...
      <div class="list">${(verificationRun.results || []).map((result) => `<div class="list-button static"><div class="list-title"><span>${escapeHTML(result.title)}</span><span>${escapeHTML(result.verdict || "")}</span></div><div class="body-copy">${escapeHTML(result.summary || "")}</div>${(result.blocked_goals || result.blockedGoals || []).length ? `<div class="muted">${escapeHTML((result.blocked_goals || result.blockedGoals || []).join("; "))}</div>` : ""}${(result.recommended_next || result.recommendedNext) ? `<div class="muted"><strong>${escapeHTML(t("verification.next"))}:</strong> ${escapeHTML(result.recommended_next || result.recommendedNext)}</div>` : ""}</div>`).join("")}</div>
    `, "card--command");
  byId("testing-side").innerHTML = `${testingCard}${verificationCard}`;
  document.querySelectorAll("[data-testing-start-step]").forEach((button) => button.addEventListener("click", async () => { await startTesting(button.dataset.testingStartStep, true); }));
  document.querySelectorAll("[data-testing-start-auto]").forEach((button) => button.addEventListener("click", async () => { await startTesting(button.dataset.testingStartAuto, false); }));
  byId("testing-side")?.querySelector("[data-testing-next]")?.addEventListener("click", async () => { await controlTesting("next"); });
  byId("testing-side")?.querySelector("[data-testing-quit]")?.addEventListener("click", async () => { await controlTesting("quit"); });
  byId("testing-side")?.querySelector("[data-testing-end]")?.addEventListener("click", async () => { await controlTesting("end"); });
  document.querySelectorAll("[data-verification-start]").forEach((button) => button.addEventListener("click", async () => { await startVerification(button.dataset.verificationStart); }));
}
...
function renderSettings() {
  ...
  byId("settings-panel").innerHTML = `
    ...
    `, "card--subtle")}
  `;
  document.querySelectorAll("[data-stop-live-app]").forEach((button) => button.addEventListener("click", async () => {
    await stopLiveApp(button.dataset.stopLiveApp);
  }));
...
function renderDisplayOverlay() {
  ...
  byId("display-scale-range")?.addEventListener("input", (event) => {
    setChatScalePercent(event.target.value);
  });
  byId("display-scale-input")?.addEventListener("change", (event) => {
    setChatScalePercent(event.target.value);
  });
  byId("display-scale-reset")?.addEventListener("click", () => {
    setChatScalePercent(100);
  });
}
...
async function openProject(pathValue) {
  ...
  window.localStorage.setItem("arc.desktop.preview.path", state.path);
...
async function initWorkspace() {
  const summary = await withBusy("Настраиваю ARC", () => postJSON("/api/init-workspace", { path: state.path, provider: "codex", mode: state.selectedAgent || "work" }), t("project.initialized"));
  state.workspace = summary;
  state.projectState = { path: summary.root, name: summary.name, state: "ready", message: t("project.initialized"), workspace: summary };
  state.showWelcome = false;
  ...
  state.currentSessionTab = "ready";
  await loadProjectData();
  showBanner(t("project.initialized"));
}

async function loadProjectData() {
  if (!isReady()) return;
  const [providers, agents, liveApps, projectMaterials, workspaceExplorer] = await Promise.all([
    transport.providers(),
    transport.agents(),
  ...
  await refreshSessions(false);
  if (state.selectedSessionId && state.sessions.some((item) => item.id === state.selectedSessionId)) await loadSessionDetail(state.selectedSessionId);
  if (state.developerAccess?.can_use_testing) {
...
async function refreshSessions(renderAfter = true) {
  if (!isReady()) { state.sessions = []; if (renderAfter) renderAll(); return []; }
  state.sessions = await transport.sessions(state.path, state.sessionSearch, state.sessionModeFilter, state.sessionStatusFilter);
...
async function loadSessionDetail(sessionID) {
  ...
  if (!state.selectedMaterialId && state.sessionDetail.materials?.length) state.selectedMaterialId = currentDocumentMaterial(state.sessionDetail)?.id || currentVisualMaterial(state.sessionDetail)?.id || state.sessionDetail.materials[0].id;
  if (sessionPollTimer) clearTimeout(sessionPollTimer);
  if (state.sessionDetail.session.status === "running") {
...
async function submitComposer() {
  if (!isReady()) { showBanner(t("project.missing"), "error"); return; }
  ...
  try {
    let detail = null;
    if (state.selectedSessionId) {
      detail = await transport.chatSend({ path: state.path, session_id: state.selectedSessionId, model: "", prompt, dry_run: false, async: true, action: "", allow_autonomy: false, attach_session_ids: state.attachedSessionIDs });
    } else {
      detail = await transport.chatStart({ path: state.path, provider, mode, model: "", prompt, dry_run: false, async: true, action: "", allow_autonomy: false, attach_session_ids: state.attachedSessionIDs });
    }
...
async function launchMaterial(materialID) {
  ...
  try {
    await withBusy(t("live.starting"), async () => {
      await transport.startMaterialLiveApp({ path: state.path, session_id: state.selectedSessionId, material_id: materialID });
      await loadSessionDetail(state.selectedSessionId);
      await refreshLiveApps(state.selectedSessionId);
...
async function openLiveApp(appID, materialID = "") {
  ...
  setLiveActionState(appID, materialID, {
    phase: "opening",
    message: state.locale === "en" ? "ARC is preparing a fresh preview inside the app." : "ARC готовит свежий preview внутри приложения.",
  });
  renderAll();
  try {
    await withBusy(t("live.starting"), async () => {
...
async function launchLessonDemo(lessonID) {
  await withBusy(t("live.starting"), async () => {
    await transport.startLessonDemo({ path: state.path, session_id: state.selectedSessionId || "", lesson_id: lessonID });
    await refreshLiveApps(state.selectedSessionId || "");
    if (state.selectedSessionId) await loadSessionDetail(state.selectedSessionId);
    renderAll();
  }, t("live.ready"));
...
async function setChatScalePercent(value) {
  state.chatScalePercent = normalizeChatScalePercent(value);
  applyChatScaleState();
  renderAll();
}
...
async function controlTesting(action, rerender = true) {
  ...
  state.testingRun = await withBusy("Выполняю шаг тестирования", () => transport.testingControl({
    path: state.path,
...
async function startVerification(profileID) {
  if (!state.developerAccess?.can_use_testing) return;
  state.verificationRun = await withBusy("Запускаю verification baseline", () => transport.startVerification({
    path: state.path,
...
function scheduleTestingPoll() {
  if (testingPollTimer) clearTimeout(testingPollTimer);
  if (!state.testingRun?.id) return;
  if (state.testingRun.status !== "running") return;
  testingPollTimer = setTimeout(async () => {
...
async function setProjectAgent(mode) {
  ...
  await withBusy(t("project.loading"), async () => {
    ...
    if (state.projectState) {
      state.projectState = { ...state.projectState, workspace: summary, message: t("project.ready") };
    }
    if (!state.selectedSessionId) state.selectedAgent = mode;
    await refreshAllowedActions();
    renderAll();
...
function renderAll() {
  renderStatic();
  document.querySelectorAll(".screen").forEach((screen) => screen.classList.toggle("active", screen.id === `screen-${state.currentScreen}`));
  renderWelcome();
...
function bindShell() {
  document.querySelectorAll(".nav-item, .topbar-nav-item").forEach((button) => button.addEventListener("click", () => {
    if (button.disabled) return;
    state.currentScreen = button.dataset.screen;
    renderAll();
  }));
  byId("show-projects-button")?.addEventListener("click", () => { state.showWelcome = true; renderWelcome(); });
  byId("reload-button")?.addEventListener("click", async () => { if (state.path) { await openProject(state.path); showBanner(t("project.refreshed")); } });
  byId("locale-select")?.addEventListener("change", (event) => { state.locale = normalizeLocale(event.target.value); window.localStorage.setItem("arc.desktop.preview.locale", state.locale); renderAll(); });
}
